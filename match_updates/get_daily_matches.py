...
from match_updates.match import Match
...
if len(matches_json) > 0:
    for entry in matches_json:
        match = Match(entry['localteam_name'], entry['visitorteam_name'], entry['time'], entry['formatted_date'],
                      entry['status'],
...
db.commit()
db.close()  # check how many users there are per match subscribed
